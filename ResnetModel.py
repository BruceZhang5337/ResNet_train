...
def resnet_Model():
    ...
    '''short_x=identity_1
    short_x= tf.keras.layers.Conv2D(filters=out_dim, kernel_size=3, padding="SAME", activation=tf.nn.relu)(short_x)
    short_x=tf.keras.layers.BatchNormalization()(short_x) '''
    for _ in range(3):
